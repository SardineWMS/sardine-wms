...
public class BinTypeController extends BaseController {
    ...
    public @ResponseBody RespObject insert(@RequestBody BinType binType) {
        RespObject resp = new RespObject();
        try {
            String uuid = binTypeService.insert(binType, getOperateContext(binType.getToken()));
            resp.setObj(uuid);
            resp.setStatus(RespStatus.HTTP_STATUS_SUCCESS);
    ...
    public @ResponseBody RespObject update(@RequestBody BinType binType) {
        RespObject resp = new RespObject();
        try {
            binTypeService.update(binType, getOperateContext(binType.getToken()));
            resp.setStatus(RespStatus.HTTP_STATUS_SUCCESS);
        } catch (IllegalArgumentException e) {
            return new ErrorRespObject("参数异常", e.getMessage());
        } catch (WMSException e) {
