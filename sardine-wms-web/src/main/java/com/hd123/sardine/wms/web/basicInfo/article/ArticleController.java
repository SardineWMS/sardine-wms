...
public class ArticleController extends BaseController {
  ...
  public @ResponseBody RespObject getByCode(@RequestParam(value = "articleCode") String articleCode,
      @RequestParam(value = "token") String token) {
    RespObject resp = new RespObject();
    try {
  ...
  @RequestMapping(value = "/getarticleqpcbyqpcstr", method = RequestMethod.GET)
  public @ResponseBody RespObject getArticleQpcByQpcStr(
      @RequestParam(value = "articleUuid", required = true) String articleUuid,
      @RequestParam(value = "qpcStr", required = true) String qpcStr) {
    RespObject resp = new RespObject();
    try {
      ArticleQpc articleQpc = articleService.getArticleQpcByArticleUuidAndQpcStr(articleUuid,
          qpcStr);
      resp.setObj(articleQpc);
      resp.setStatus(RespStatus.HTTP_STATUS_SUCCESS);
    } catch (Exception e) {
      return new ErrorRespObject("查询商品规格失败：" + e.getMessage());
    }
    return resp;
