...
public class BinController extends BaseController {
    ...
    public @ResponseBody RespObject insertShelf(
        ...
        try {
            ApplicationContextUtil.setCompany(getLoginCompany(token));
            binService.insertShelf(pathCode, getLoginCompany(token).getUuid(),
    ...
    public @ResponseBody RespObject delete(
            @RequestParam(value = "uuid", required = true) String uuid,
            @RequestParam(value = "version", required = false) long version,
            @RequestParam(value = "token", required = true) String token) {
        RespObject resp = new RespObject();
        try {
            binService.remove(uuid, version, getLoginCompany(token).getUuid(),
                    getOperateContext(token));
            resp.setStatus(RespStatus.HTTP_STATUS_SUCCESS);
        } catch (Exception e) {
    ...
    public @ResponseBody RespObject getBinByCode(
            @RequestParam(value = "bincode", required = true) String bincode,
            @RequestParam(value = "token", required = true) String token) {
        RespObject resp = new RespObject();
        try {
            ApplicationContextUtil.setCompany(getLoginCompany(token));
