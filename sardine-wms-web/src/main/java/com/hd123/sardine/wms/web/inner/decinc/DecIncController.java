...
public class DecIncController extends BaseController {
  ...
  @RequestMapping(value = "refreshcaseqtystrandamount", method = RequestMethod.PUT)
  public RespObject refreshCaseQtyStrAndAmount(
      @RequestParam(value = "token", required = true) String token,
      @RequestBody DecIncInvBill bill) {
    RespObject resp = new RespObject();
    try {
      ...
      for (DecIncInvBillItem item : bill.getItems()) {
        item.setLine(++i);
        if (Objects.equals(BigDecimal.ZERO, item.getQty()) || Objects.isNull(item.getArticle()))
          continue;
      ...
      bill.setTotalCaseQtyStr(totalCaseQtyStr);
      resp.setObj(bill);
      resp.setStatus(RespStatus.HTTP_STATUS_SUCCESS);
      return resp;
    } catch (Exception e) {
      return new ErrorRespObject("刷新件数和金额失败：", e.getMessage());
    }
