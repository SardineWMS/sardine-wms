...
public class SerialArchDaoImpl extends BaseDaoImpl<SerialArch> implements SerialArchDao {
  ...
  public List<SerialArchLineCustomer> queryCustomerByLine(PageQueryDefinition definition) {
    ...
    return getSqlSession().selectList(generateStatement(MAPPER_QUERYCUSTOMERBYLINE), definition);
  }

  @Override
  public List<SerialArchLine> getLineByCompanyUuid() {
    String companyUuid = ApplicationContextUtil.getCompanyUuid();
    return getSqlSession().selectList(generateStatement(MAPPER_GETLINEBYCOMPANYUUID), companyUuid);
  }
