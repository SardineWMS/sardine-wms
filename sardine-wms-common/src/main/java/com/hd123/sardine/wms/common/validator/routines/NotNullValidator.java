 ...
 * @author zhangsai
 */
public class NotNullValidator extends GenericBeanValidator<Object> {

    @Override
    public boolean doValidate(ValidatorContext context, Object obj) {
        if (obj == null) {
            context.addError(IllegalArgumentErrorMsg.create(MessageFormat.format("{0}不能为空", getFieldCaption())));
            return false;
        }
