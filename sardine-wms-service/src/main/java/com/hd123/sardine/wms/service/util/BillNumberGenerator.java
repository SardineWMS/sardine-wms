...
public class BillNumberGenerator {
    ...
    public String allocateNextPathCode(String zoneCode) {
        ...
        if (StringUtil.isNullOrBlank(maxPathCode)) {
            ...
            nextPathCode = String.format("%0" + PATHCODE_LENGTH + "d",
                    Long.valueOf(maxPathCode) + 1);
    ...
    public String allocateCompanyUuid() {
        String maxCompanyUuid = billNumberDao
                .getMaxSequenceValueForUpdate(DEFAULT_COMPANY_SEQUENCENAME, DEFAULT_COMPANYUUID);
        String nextCompanyUuid = null;
        if (StringUtil.isNullOrBlank(maxCompanyUuid)) {
            billNumberDao.getMaxSequenceValueForUpdate(DEFAULT_SEQENCENAME, DEFAULT_SEQENCENAME);
            maxCompanyUuid = billNumberDao.getMaxSequenceValueForUpdate(
                    DEFAULT_COMPANY_SEQUENCENAME, DEFAULT_COMPANYUUID);
        }
    ...
    public String allocateNextWareHouseUuid() {
        ...
        String maxWarehouseUuid = billNumberDao.getMaxSequenceValueForUpdate(companyUuid,
                companyUuid);
        ...
        if (StringUtil.isNullOrBlank(maxWarehouseUuid)) {
            nextWarehouseUuid = ApplicationContextUtil.getParentCompanyUuid()
    ...
    public String allocateNextContainerBarcode(String prefix, int length) {
        ...
        String companyUuid = ApplicationContextUtil.getCompanyUuid();
        String maxContainerBarcode = billNumberDao.getMaxSequenceValueForUpdate(prefix,
                companyUuid);
        ...
        if (StringUtil.isNullOrBlank(maxContainerBarcode)) {
            nextContainerBarcode = prefix
                    + String.format("%0" + length + "d", Long.valueOf(START_CONTAINERBARCODE));
    ...
    public String allocateNextBillNumber(String sequenceName) {
        ...
        String companyUuid = ApplicationContextUtil.getCompanyUuid();
        String maxSequenceValue = billNumberDao.getMaxSequenceValueForUpdate(sequenceName,
                companyUuid);
        if (StringUtil.isNullOrBlank(maxSequenceValue)) {
            billNumberDao.getMaxSequenceValueForUpdate(DEFAULT_SEQENCENAME, DEFAULT_SEQENCENAME);
            maxSequenceValue = billNumberDao.getMaxSequenceValueForUpdate(sequenceName,
                    companyUuid);
        }
        ...
        if (StringUtil.isNullOrBlank(maxSequenceValue)) {
            ...
            if (maxSequenceValue
                    .indexOf(ApplicationContextUtil.getCompanyCode() + dateString) >= 0) {
                nextSequenceValue = String
                        .valueOf(new BigInteger(maxSequenceValue).add(BigInteger.ONE));
                billNumberDao.updateSequenceValue(sequenceName, nextSequenceValue, companyUuid);
            } else {
                nextSequenceValue = ApplicationContextUtil.getCompanyCode() + dateString
                        + FIRST_NUMBER;
                billNumberDao.updateSequenceValue(sequenceName, nextSequenceValue, companyUuid);
            }
        }
