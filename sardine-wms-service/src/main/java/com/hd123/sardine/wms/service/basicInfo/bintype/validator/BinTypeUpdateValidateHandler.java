 ...
 * 文件名：	BinTypeUpdateValidateHandler.java
 * 模块说明：	
 * 修改历史：
 ...
 *
 */
public class BinTypeUpdateValidateHandler extends GenericValidateHandler<BinType> {
    public static final String KEY_CODEEXISTS_BINTYPE = "codeExists_binType";
    public static final String KEY_UPDATE_BINTYPE = "update_binType";
    ...
    protected void prepareValidators(BinType bean) {
        ...
        on("货位类型uuid", bean.getUuid(), notNullValidator);
        ...
        // on("货位类型容积率", bean.getPlotRatio(), notNullValidator);
        // on("货位类型容积率", bean.getPlotRatio(), ratioValidator);
        on("货位类型承重", bean.getBearing(), notNullValidator);
        on("货位类型承重", bean.getBearing(), nonnegativeNumberValidator);

        BinType binType = (BinType) getAttribute(KEY_CODEEXISTS_BINTYPE);
        on("货位类型" + bean.getCode(), binType, nullValidator);

        BinType updateBinType = (BinType) getAttribute(KEY_UPDATE_BINTYPE);
        on("编辑的货位类型", updateBinType, entityNotNullValidator);

        on(updateBinType, versionValidator);
