...
public class WaveBillServiceImpl extends BaseWMSService implements WaveBillService {
    ...
    public String saveNew(WaveBill bill) throws IllegalArgumentException, WMSException {
        ...
        bill.setSerialArch(
                new UCN(serialArch.getUuid(), serialArch.getCode(), serialArch.getName()));

        bill.setBillNumber(billNumberGenerator.allocate(Constants.WAVEBILL_NUMBER_TYPE));
    ...
    public void saveModify(WaveBill bill)
        ...
        PersistenceUtils.checkVersion(bill.getVersion(), waveBill, WaveBill.CAPTION,
                bill.getUuid());
        if (WaveBillState.initial.equals(bill.getState()) == false)
            throw new WMSException(MessageFormat.format("该波次单的状态是{0}，不能编辑，只有状态的补货单才能拿编辑",
                    waveBill.getState().getCaption()));
        SerialArch serialArch = serialArchService.get(bill.getSerialArch().getUuid());
        bill.setSerialArch(
                new UCN(serialArch.getUuid(), serialArch.getCode(), serialArch.getName()));
        bill.setState(WaveBillState.initial);
