...
public class ScopeUtils {
     ...
     *            查询前缀
     * @param scopeExp
     *            货位范围表达式，多个条件使用“,”分割，装换为SQL语句为“or”，范围使用“-”，解释为SQL语句为between...
     *            and ...语句
     * @return 返回SQL语句
     */
    ...
    public static boolean contains(String binScope, String binCode) {
        ...
        for (String scope : scopes) {
            ...
            if (scope.indexOf("-") > -1) {
                String[] items = scope.split("-");
                String startCode = items[0].length() < binCode.length()
                        ? items[0] + StringUtils.repeat("0", binCode.length() - items[0].length())
                        : items[0];
                String endCode = items[1].length() < binCode.length()
                        ? items[1] + StringUtils.repeat("9", binCode.length() - items[1].length())
                        : items[1];
    ...
    private static String scopeExpToSQLExp2(String alias, String scope) {
        ...
        if (items.length > 1) {
            ...
            if (start.compareTo(end) > 0)
                ...
                result += "(" + alias + " between '" + items[0] + "%' and '" + items[1] + "%' or "
                        + alias + " like '" + items[1] + "%' or " + alias + " like '" + items[0]
                        + "%')";
        } else {
