 ...
 * <p>
 * <ul>
 * <li>待编辑的用户uuid不能为空，且用户必须存在</li>
 * <li>版本号一致</li>
 * <li>新用户的基本信息必须满足规范</li>
 * <li>新用户代码不能重复，需由调用者将根据新用户代码查询出的用户以key
 * {@link UserUpdateValidateHandler#KEY_CODEEXISTS_USER}传入</li>
 * </ul>
 * 
 * @author zhangsai
 *
 */
public class UserUpdateValidateHandler extends GenericValidateHandler<User> {

    /** 调用该校验器的service需传入user对象 与当前注册用户代码一致的已存在用户 */
    public static final String KEY_CODEEXISTS_USER = "codeExists_user";

    /** 调用该校验器的service需传入当前待编辑的user对象 */
    public static final String KEY_UPDATE_USER = "update_user";
    ...
    protected void prepareValidators(User bean) {
        ...
        on("用户代码", bean.getUuid(), notNullValidator);
        ...
        User updateUser = (User) getAttribute(KEY_UPDATE_USER);
        on("编辑的用户", updateUser, entityNotNullValidator);

        on(updateUser, versionValidator);
    }
}
