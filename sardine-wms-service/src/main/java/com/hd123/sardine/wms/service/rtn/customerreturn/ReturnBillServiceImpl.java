...
public class ReturnBillServiceImpl extends BaseWMSService implements ReturnBillService {
	...
	public String saveNew(ReturnBill bill) throws WMSException, ParseException {
		...
		bill.setBillNumber(billNumberGenerator.allocateNextBillNumber(ReturnBill.class.getSimpleName()));
	...
	private void verifyAndRefreshBill(ReturnBill bill) throws WMSException, ParseException {
		...
		if ((ReturnNtcBillState.initial.equals(returnNtcBill.getState())
			...
			throw new WMSException(
					MessageFormat.format("退仓通知单状态是{0}，不是初始或进行中的，不能操作", returnNtcBill.getState().getCaption()));
	...
	private void verifyContainers(StringBuffer errorMsg, ReturnBill bill) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (ContainerState.STACONTAINERIDLE.equals(container.getState()) == false) {
				errorMsg.append(MessageFormat.format("第{0}行，容器状态是{1}，不是空闲状态", i, container.getState().getCaption()));
				continue;
			}
	...
	private void verifyAndRefreshArticles(StringBuffer errorMsg, List<ReturnBillItem> items) throws ParseException {
		assert errorMsg != null;
		assert items != null;
		...
		for (ReturnBillItem item : items) {
			...
			if (Objects.isNull(article))
				errorMsg.append(MessageFormat.format("第{0}行中，商品{1}不存在", item.getLine(), item.getArticle().getCode()));
			item.setProductionDate(article.calProductionDate(item.getProductionDate(), item.getValidDate()));
	...
	private StringBuffer verifyBinAndRefreshItems(ReturnBill bill, ReturnNtcBill returnNtcBill) throws WMSException {
		assert bill != null;
		...
		if (Objects.isNull(bin))
			throw new WMSException(MessageFormat.format("当前仓位{0}下没有退仓收货暂存位，无法进行退仓 ", bill.getWrh().getCode()));
		StringBuffer errorMsg = new StringBuffer();
		for (ReturnBillItem item : bill.getItems()) {
	...
	public void saveModify(ReturnBill bill) throws WMSException, ParseException {
		...
		if (ReturnBillState.initial.equals(existBill.getState()) == false)
			throw new WMSException(MessageFormat.format("当前退仓单的状态是{0}，不是初始状态，不能修改", bill.getState().getCaption()));
	...
	public void remove(String uuid, long version) throws WMSException {
		...
		if (ReturnBillState.initial.equals(bill.getState()) == false)
			throw new WMSException(MessageFormat.format("当前退仓单的状态是{0}，不是初始，不能删除", bill.getState().getCaption()));
		dao.remove(uuid, version);
		dao.removeItems(uuid);

		logger.injectContext(this, uuid, ReturnBill.class.getName(), ApplicationContextUtil.getOperateContext());
	...
	public void audit(String uuid, long version) throws WMSException {
		...
		if (ReturnBillState.initial.equals(bill.getState()) == false)
			throw new WMSException(MessageFormat.format("当前退仓单的状态是{0}，不是初始状态，不能审核", bill.getState().getCaption()));
		StringBuffer errorMsg = new StringBuffer();
	...
	private void shiftIn(ReturnBill bill) throws WMSException {/*
		...
		for (ReturnBillItem item : bill.getItems()) {
			...
			stock.setCompanyUuid(bill.getCompanyUuid());
			stocks.add(stock);
		}
	...
	*/}
	...
	private StringBuffer verifyContainer(ReturnBill bill) {
		...
		for (ReturnBillItem item : bill.getItems()) {
			Container container = containerService.getByBarcode(item.getContainerBarcode());
			if (Objects.isNull(container)) {
				errorMsg.append(MessageFormat.format("第{0}行中的容器{1}不存在", item.getLine(), item.getContainerBarcode()));
				continue;
			}
			if (Container.VIRTUALITY_CONTAINER.equals(item.getContainerBarcode())) {
	...
	private void verifyItemQty(StringBuffer errorMsg, ReturnBill bill) throws WMSException {
		...
		if (Objects.isNull(ntcBill))
			throw new WMSException(MessageFormat.format("退仓单对应的退仓通知单{0}不存在", bill.getReturnNtcBillNumber()));
		...
		for (ReturnBillItem item : bill.getItems()) {
			...
			if (Objects.equals(item.getArticle().getUuid(), ntcBillItem.getArticle().getUuid()) == false) {
