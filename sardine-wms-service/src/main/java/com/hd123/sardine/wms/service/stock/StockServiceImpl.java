...
public class StockServiceImpl implements StockService {
    ...
    public List<Stock> shiftIn(String sourceBillType, String sourceBillNumber,
        ...
        for (Stock stock : willInStocks) {
            stock.validate();
            Stock existsStock = stockDao.getByPrimaryKey(stock.getBinCode(),
                    stock.getContainerBarcode(), stock.getStockBatch(), stock.getArticleUuid());
            if (existsStock != null) {
                if (stock.getQty().compareTo(BigDecimal.ZERO) > 0) {
                    existsStock.setQty(existsStock.getQty().add(stock.getQty()));
                    log(existsStock, stock.getQty(), StockLog.NORMAL_LOG, sourceBillType,
                            sourceBillNumber);
                } else {
                ...
                for (OnWayStock onWayStock : stock.getOnWayStocks()) {
                    ...
                    log(stock, onWayStock.getQty(), StockLog.ONWAY_LOG, sourceBillType,
                            sourceBillNumber);
                    stockDao.insertOnWayStock(onWayStock);
                }
    ...
    public List<Stock> shiftInOnWayStock(List<OnWayStock> onWayStocks)
        ...
        for (OnWayStock onWayStock : onWayStocks) {
            ...
            if (StringUtil.isNullOrBlank(onWayStock.getStockUuid()) == false) {
                ...
                stock = stockDao.getByPrimaryKey(onWayStock.getBinCode(),
                        onWayStock.getContainerBarcode(), onWayStock.getStockBatch(),
                        onWayStock.getArticleUuid());
            }
            if (stock == null) {
                stock = new Stock();
                stock.setUuid(UUIDGenerator.genUUID());
            ...
            log(stock, onWayStock.getQty(), StockLog.ONWAY_LOG,
                    onWayStock.getTaskType().getCaption(), onWayStock.getTaskNo());
        }
    ...
    public List<Stock> shiftOutOnWayStock(List<OnWayStockOutRule> shiftRules)
        ...
        for (OnWayStockOutRule outRule : shiftRules) {
            ...
            if (stock == null)
                stock = stockDao.getByPrimaryKey(outRule.getBinCode(),
                        outRule.getContainerBarcode(), outRule.getStockBatch(),
                        outRule.getArticleUuid());
            if (stock == null)
